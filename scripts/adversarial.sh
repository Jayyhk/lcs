    ...
    MEM_LIMIT_MB=2048          # 2GiB in MiB
    SWAP_LIMIT=2147483648      # 2GiB

    # Temp log files
    HIRSCHBERG_LOG="res/adversarial/hirschberg.log"
    OBLIVIOUS_LOG="res/adversarial/oblivious.log"
    ...
    for N in 32768 65536 131072; do
        ...
        ./bin/lcs_hirschberg_instrumented $N 1 $BASE_CASE < "rsrc/data-$N.in" > /dev/null 2> "res/adversarial/profiles/scan_$N.log"
        ...
        awk 'BEGIN { FS=","; print "0.0 4" } /START_SCAN/ { print $1 " 2048" } /END_SCAN/ { print $1 " 4" }' \
            "res/adversarial/profiles/scan_$N.log" > "res/adversarial/profiles/adversarial_$N.txt"
        
        # Phase 2: Run the experiment
        # Point to the profile file created above
        PROFILE_FILE="res/adversarial/profiles/adversarial_$N.txt"
        ...
        stdbuf -o0 cgexec -g memory:$CGROUP_NAME ./bin/balloon 1 \
            $MEM_LIMIT_MB 4 1 1 $PROFILE_FILE > res/adversarial/balloon.log 2>&1 &
        BALLOON_PID=$!
