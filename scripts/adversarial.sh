...
mkdir -p res/adversarial/balloon # Ensure directory exists for balloon
...
for N in 32768 65536 131072; do
    ...
    ./bin/lcs_hirschberg_instrumented $N 1 $BASE_CASE < "rsrc/data-$N.in" > /dev/null 2> "res/adversarial/balloon/scan_$N.log"
    
    # Phase 1: Generate adversarial profile from log
    echo "  Generating adversarial profile..."
    awk 'BEGIN { FS=","; print "0.0 2048" } /START_SCAN/ { print $1 " 4" } /END_SCAN/ { print $1 " 2048" }' \
        "res/adversarial/balloon/scan_$N.log" > "res/adversarial/balloon/adversarial_$N.txt"
    
    # Phase 2: Run the experiment
    # Point to the profile file created above
    PROFILE_FILE="res/adversarial/balloon/adversarial_$N.txt"
    ...
    stdbuf -o0 cgexec -g memory:$CGROUP_NAME ./bin/balloon 1 \
        $MEM_LIMIT_MB 2048 1 1 $PROFILE_FILE > res/adversarial/balloon/balloon_$N.log 2>&1 &
    BALLOON_PID=$!
    ...
    stdbuf -o0 cgexec -g memory:$CGROUP_NAME ./bin/balloon 1 \
        $MEM_LIMIT_MB 2048 1 1 $PROFILE_FILE > res/adversarial/balloon/balloon_oblivious_$N.log 2>&1 &
    BALLOON_PID=$!
